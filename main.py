...
def calculate_indicators(df: pd.DataFrame):
    ...
    # Bollinger Bands (для фильтрации флэта)
...
def analyze_timeframe(df: pd.DataFrame, direction: str = "LONG"):
    ...
    # Минимальная ширина Боллинджера (исключаем флэт)
    if last["bb_width"] < 0.005:  # можно подстроить
        return False
...
def scan_signals():
    ...
    for symbol in symbols[:50]:  # лимит, чтобы не превысить лимиты API
        # 1. Анализ на H1
...
if __name__ == "__main__":
    send_telegram_message("✅ Бот запущен! Сканирование каждые 5 мин.")
